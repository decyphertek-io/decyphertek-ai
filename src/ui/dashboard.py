                        ...
                        content=ft.Row(
                            controls=[
                                ...
                                ft.IconButton(
                                    ...
                                    tooltip="Refresh",
                                    on_click=lambda e: self._refresh_mcp_tab()
                                ),
                            ]
                        ),
                        padding=15,
                    ...
                    ft.Container(
                        content=ft.Column([
                            ...
                            ft.Row([
                                ...
                                ft.Container(expand=True),
                                ft.Icon(
                                    ft.icons.CHECK_CIRCLE if self._is_smithery_configured() else ft.icons.WARNING,
                                    size=20,
                                    color=ft.colors.GREEN if self._is_smithery_configured() else ft.colors.ORANGE
                                ),
                            ]),
                            ...
                            ft.Container(
                                content=ft.Column([
                                    ft.Row([
                                        ...
                                        ft.Text(
                                            "Connected" if self._is_smithery_configured() else "Not Configured",
                                            size=12,
                                            weight=ft.FontWeight.BOLD,
                                            color=ft.colors.GREEN if self._is_smithery_configured() else ft.colors.ORANGE
                                        ),
                            ...
                            # Available Servers from Store
                            ft.Row([
                                ft.Text("Available Servers", size=14, weight=ft.FontWeight.BOLD),
                                ft.Container(expand=True),
                                ft.Text(
                                    f"{len(self._get_connected_servers())}/{len(self._get_available_servers())} connected",
                                    size=12,
                                    color=ft.colors.GREY_600
                                ),
                            ]),
                            ...
                            # Server cards from MCP store
                            ft.Column([
                                self._create_mcp_server_card(
                                    server["name"],
                                    server["description"],
                                    server["icon"],
                                    server["color"],
                                    server["id"],
                                    connected=server["id"] in self._get_connected_servers()
                                ) for server in self._get_available_servers()
                            ] + [ft.Container(height=10)] if self._get_available_servers() else [
                                ft.Container(
                                    content=ft.Column([
                                        ft.Icon(ft.icons.CLOUD_OFF, size=60, color=ft.colors.GREY_400),
                                        ft.Container(height=10),
                                        ft.Text("No servers loaded", size=14, color=ft.colors.GREY_600),
                                        ft.Container(height=5),
                                        ft.Text("Configure custom store above", size=11, color=ft.colors.GREY_500),
                                    ], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                                    padding=40,
                                    alignment=ft.alignment.center
                                )
